...
Compared to using a for loop, focus is on the operation being performed, not the bookkeeping required to loop over every element and store the output. This is even more apparent if we use the pipe:
...
Functions in purrr have more consistent names and arguments, helpful shortcuts, and in the future will provide easy parallelism and progress bars.
...
That generates longer, but safer, calls. Since the arguments are all the same length, it makes sense to store them in a data frame:
