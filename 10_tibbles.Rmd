...
To revise, you can create/coerce a tibble with `as_tibble`, `tibble` and now also `tribble` — short for transposed tibble. The function is customised for data entry in code: column headings are defined by formulas (i.e. they start with `~`), and entries are separated by commas. You can literally layout the table you want, and `tribble` will coerce it for you. I often add a comment (the line starting with #), to make it really clear where the header is.
```{r}
tribble(
  ~x, ~y, ~z,
  #--|--|----
