...
To match a `.` in regexp, we need to use a double escape, `\\`. This is because regexp use `\` as an escape character, but `\` is also used as an escape character in strings, which are used to represent regexp. 
